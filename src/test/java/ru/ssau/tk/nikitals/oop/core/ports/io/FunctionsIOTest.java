...
class FunctionsIOTest {
    ...
    void testSerializeAndDeserializeJson() throws IOException {
        ...
        ArrayTabulatedFunction deserializedFunction;
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath.toFile()))) {
            deserializedFunction = FunctionsIO.deserializeJson(reader, ArrayTabulatedFunction.class);
        }
        assertEquals(function.toString(), deserializedFunction.toString());
    }

    @Test
    void testLinkedListSerializeAndDeserializeJson() throws IOException {
        LinkedListTabulatedFunction function = new LinkedListTabulatedFunction(xValues, yValues);

        Path filePath = TEMP_DIR.resolve("linkedListFunction.json");
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath.toFile()))) {
            FunctionsIO.serializeJson(writer, function);
        }

        LinkedListTabulatedFunction deserializedFunction;
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath.toFile()))) {
            deserializedFunction = FunctionsIO.deserializeJson(reader, LinkedListTabulatedFunction.class);
        }
        assertEquals(function.toString(), deserializedFunction.toString());
    }
